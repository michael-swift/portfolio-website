...
export default async function SciencePage({ params }: SciencePageProps) {
  ...
  const science = await getContentItem('science', slug)
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        >
          <div dangerouslySetInnerHTML={{ __html: science.content }} />
        </article>
...
// Generate static paths for all science items
export async function generateStaticParams() {
  const science = await getContentItems('science')
  
  return science.map((item) => ({
    slug: item.slug,
  }))
}
...
export async function generateMetadata({ params }: SciencePageProps) {
  ...
  if (!science) {
    return {
      title: 'Science Item Not Found'
    }
  }

  return {
    title: `${science.title} | Michael Swift`,
    description: science.excerpt || science.content.substring(0, 160) + '...',
  }
}